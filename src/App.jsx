...
const BLOG_POSTS = [
  { id: 1, tag: "GxP · CSV", date: "Mar 2024", readTime: "6 min read", icon: "🔬", color: "#00f5c4",
    ...
    fix: "I introduced an environment-assertion step at the very top of every automation script — it reads the active system URL, validates it against a config-driven allowlist, and fails loudly with a blocking error if there's a mismatch. I also updated the URS traceability matrix to require explicit environment sign-off before any IQ/OQ/PQ run. The client passed their FDA inspection with zero 483 observations related to CSV.",
  ...
  { id: 2, tag: "Automation · Flaky Tests", date: "Jan 2024", readTime: "5 min read", icon: "⚡", color: "#7b61ff",
    ...
    root: "Three compounding issues: (1) implicit waits mixed with explicit waits causing unpredictable timing behaviour, (2) tests sharing mutable global state through static session objects, (3) hardcoded pixel-based element locators breaking when the UI was responsive.",
    fix: "I replaced all implicit waits with a custom ExpectedConditions wrapper using exponential backoff. Shared state was eliminated by introducing a ThreadLocal WebDriver factory. Locators were migrated to data-testid attributes added in coordination with the frontend team. Flakiness dropped from 35% to under 2% in three weeks.",
  ...
  { id: 3, tag: "API Testing · Security", date: "Nov 2023", readTime: "4 min read", icon: "🔒", color: "#ff6b35",
    ...
    fix: "I added a negative assertion layer to every API test: alongside verifying expected fields, tests now explicitly assert that sensitive fields are absent from responses. I introduced a JSON schema validation step using Ajv in the Cypress suite. The security fix was patched same-day and the schema tests now live as a permanent gate in the CI pipeline.",
  ...
  { id: 4, tag: "Performance · Database", date: "Sep 2023", readTime: "7 min read", icon: "📈", color: "#ffd700",
    ...
    root: "JMeter load tests confirmed the degradation. Digging into SQL Server profiler traces revealed N+1 query patterns on the sample retrieval endpoint — each sample record was triggering 8 additional sub-queries rather than a single JOIN. At 50 users this was generating ~2,000 queries per second against one table.",
...
// QAIX intro script — cinematic subtitles synced to robot animation
const QAIX_SCRIPT = [
...
const INTRO_DURATION = 18000; // total ms before auto-skip
...
const QAIX_RESPONSES = {
  ...
  hire: "Guru is open to QA opportunities — especially in regulated industries (pharma, medtech, fintech). Hit Contact or reach out on GitHub! 📬",
  ...
  default: "I can tell you about Guru's tools, projects, GxP expertise, experience, or whether he's available for hire. What would you like to know? 🤔",
};
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      dots.forEach((d) => {
        ...
        ctx.fillStyle = "rgba(0,245,196,0.35)"; ctx.fill();
      });
      dots.forEach((a, i) => dots.slice(i + 1).forEach((b) => {
        const dist = Math.hypot(a.x - b.x, a.y - b.y);
        if (dist < 120) {
          ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y);
          ctx.strokeStyle = `rgba(0,245,196,${0.12 * (1 - dist / 120)})`; ctx.lineWidth = 0.5; ctx.stroke();
        }
      }));
  ...
  return (
    <svg viewBox="0 0 240 280" className="w-64 h-72 md:w-80 md:h-96 drop-shadow-[0_0_60px_rgba(0,245,196,0.3)]">
      ...
      >
        ...
        {/* Eyes */}
        <motion.g
          animate={speaking ? { scaleY: [1, 0.2, 1, 1] } : { scaleY: [1, 1, 0.15, 1, 1] }}
          transition={{ repeat: Infinity, duration: speaking ? 2 : 4, times: speaking ? [0, 0.45, 0.5, 1] : [0, 0.45, 0.5, 0.55, 1] }}
        ...
        <motion.rect
          x="100" y="135" width="40" height="6" rx="3"
          fill="#00f5c4"
          animate={speaking ? {
            height: [6, 14, 8, 16, 6, 12, 6],
            y: [135, 131, 134, 130, 135, 132, 135]
          } : { height: 6, y: 135 }}
      ...
      <line x1="105" y1="167" x2="135" y2="167" stroke="#00f5c4" strokeWidth="0.5" opacity="0.5" />
      ...
      <motion.g animate={{ y: [0, -2, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut", delay: 0.2 }}>
        ...
        {/* Heart pulse / chest light */}
        <motion.circle
          cx="120" cy="210" r="6" fill="#00f5c4"
          animate={{ scale: [1, 1.3, 1], opacity: [0.7, 1, 0.7] }}
          transition={{ repeat: Infinity, duration: 1.2 }}
        />
        {/* Body grid lines */}
        <line x1="80" y1="240" x2="160" y2="240" stroke="#00f5c4" strokeWidth="0.5" opacity="0.3" />
      </motion.g>
    </svg>
  );
}

// ─── CINEMATIC INTRO (replaces login) ─────────────────────────────────────────
...
function CinematicIntro({ onEnd }) {
  ...
  // Drive subtitle progression
  useEffect(() => {
    if (!started) return;
    const timers = QAIX_SCRIPT.map(({ t }, i) =>
      setTimeout(() => setCurrentLine(i), t)
    );
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        {!started ? (
          ...
          >
            ...
            <p className="font-mono text-sm text-[#8892a4] tracking-widest mb-12">
              QA ENGINEER · GxP · CSV · AUTOMATION
            </p>
            ...
            <button
              onClick={onEnd}
              className="block mx-auto mt-6 font-mono text-xs text-[#8892a4] hover:text-white transition-colors underline underline-offset-4"
            >
              Skip intro →
            </button>
          </motion.div>
          ...
          >
            ...
            >
              <span>&lt;</span>
              <span className="tracking-[0.4em]">QAIX.AI</span>
              <span>/&gt;</span>
            </motion.div>
      ...
      {/* Skip button — shows after intro starts */}
      <AnimatePresence>
        {started && showSkip && (
          <motion.button
...
// ─── QAIX CHAT (floating, opens on demand) ────────────────────────────────────

function QaixChat({ open, onClose }) {
  const [chatInput, setChatInput] = useState("");
  const [chatHistory, setChatHistory] = useState([
    { from: "qaix", text: "Hey! I'm QAIX. Ask me anything about Guru — his tools, projects, GxP work, or whether he's available for hire." }
  ]);
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#00f5c4] animate-pulse" />
          ...
          <div className="h-72 overflow-y-auto px-4 py-3 space-y-3">
            {chatHistory.map((m, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, x: m.from === "user" ? 16 : -16 }}
              ...
              >
                ...
                <div className={`rounded-xl px-3 py-2 text-xs leading-relaxed max-w-[80%] ${
                  m.from === "user"
                    ...
                    : "bg-[#0d1520] border border-white/5 text-[#c8d0dc] rounded-tl-none"
                }`}>{m.text}</div>
              </motion.div>
            ))}
            {typing && (
          ...
          <div className="px-3 py-3 border-t border-white/5 bg-[#0d1520] flex gap-2">
            <input
              value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={handleKey}
...
function Section({ id, children, className = "" }) {
  ...
  return (
    <section id={id} ref={ref} className={`relative z-10 py-28 px-6 md:px-16 lg:px-32 ${className}`}>
      <motion.div initial={{ opacity: 0, y: 48 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
        {children}
      </motion.div>
    </section>
  );
...
function Navbar() {
  ...
  return (
    <motion.nav
      initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-8">
        {NAV_LINKS.map((l) => (
          <button key={l} onClick={() => scrollTo(l)} className="font-mono text-sm text-[#8892a4] hover:text-[#00f5c4] transition-colors duration-200 tracking-wider">{l}</button>
        ))}
        <a href="https://github.com/Guruprasad-Chougule" target="_blank" rel="noreferrer"
          className="px-4 py-2 border border-[#00f5c4]/50 text-[#00f5c4] font-mono text-sm rounded hover:bg-[#00f5c4]/10 transition-colors">GitHub ↗</a>
      </div>
...
function Hero() {
  const typed = useTypewriter(["QA Engineer", "GxP Specialist", "CSV Validator", "Automation Expert"]);
  const { scrollY } = useScroll();
  const y = useTransform(scrollY, [0, 600], [0, 120]);
  return (
    <section id="hero" className="relative z-10 min-h-screen flex flex-col justify-center px-6 md:px-16 lg:px-32">
      <motion.div style={{ y }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7, ease: [0.22, 1, 0.36, 1] }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }} className="flex items-center gap-3 mb-8">
          <span className="font-mono text-xl md:text-2xl text-[#8892a4]">{typed}<span className="animate-pulse text-[#00f5c4]">|</span></span>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5 }} className="flex flex-wrap gap-4">
          <button onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="px-8 py-3.5 bg-[#00f5c4] text-[#080c10] font-mono font-bold text-sm rounded tracking-widest hover:shadow-[0_0_30px_rgba(0,245,196,0.4)] transition-all">Get In Touch →</button>
          <button onClick={() => document.getElementById("projects")?.scrollIntoView({ behavior: "smooth" })}
            className="px-8 py-3.5 border border-white/20 text-white font-mono text-sm rounded tracking-widest hover:border-[#00f5c4]/50 hover:text-[#00f5c4] transition-all">View Projects</button>
        </motion.div>
      </motion.div>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }}
        className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
        <span className="font-mono text-xs text-[#8892a4] tracking-widest">SCROLL</span>
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-px h-10 bg-gradient-to-b from-[#00f5c4] to-transparent" />
      </motion.div>
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}
      ...
      <div className="p-6">
        ...
        <div className="flex gap-2 mb-1">
          <span className="font-mono text-xs text-red-400 shrink-0 mt-0.5">⚠ PROBLEM</span>
        </div>
  ...
  return (
    <Section id="contact" className="bg-[#050810]">
      ...
      <div className="grid md:grid-cols-2 gap-16">
        <div>
          <p className="text-[#8892a4] leading-relaxed mb-8">Whether you're looking for a QA specialist, need GxP consultation, or just want to talk quality engineering — my inbox is always open.</p>
          <div className="space-y-4">
            {[
              { label: "GitHub", val: "Guruprasad-Chougule", href: "https://github.com/Guruprasad-Chougule" },
              { label: "Portfolio", val: "guruprasadchougule.vercel.app", href: "https://guruprasadchougule.vercel.app" },
              { label: "Location", val: "India 🇮🇳", href: null },
            ].map(({ label, val, href }) => (
              <div key={label} className="flex gap-4 items-center">
                <span className="font-mono text-xs text-[#8892a4] w-20 shrink-0">{label}</span>
                {href ? <a href={href} target="_blank" rel="noreferrer" className="text-[#00f5c4] hover:underline font-mono text-sm">{val}</a>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[{ id: "name", label: "Name", type: "text", placeholder: "Your name" }, { id: "email", label: "Email", type: "email", placeholder: "your@email.com" }].map(({ id, label, type, placeholder }) => (
            <div key={id}>
              ...
              <input type={type} value={form[id]} onChange={(e) => setForm((f) => ({ ...f, [id]: e.target.value }))} placeholder={placeholder} required
          ...
          <div>
            ...
            <textarea rows={5} value={form.message} onChange={(e) => setForm((f) => ({ ...f, message: e.target.value }))} placeholder="Tell me about your project or opportunity..." required
          ...
          {status === "error" && <p className="text-red-400 font-mono text-xs text-center">Something went wrong. Try GitHub instead.</p>}
        </form>
      </div>
    </Section>
  );
...
function Footer() {
  return (
    <footer className="relative z-10 border-t border-white/5 px-6 md:px-16 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <span className="font-mono text-xs text-[#8892a4]">Designed & Developed with <span className="text-[#00f5c4]">♥</span></span>
    </footer>
  );
}
